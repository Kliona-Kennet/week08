...
document.addEventListener('DOMContentLoaded', () => {
    const PRODUCT_API_BASE_URL = 'http://4.198.191.223:8000/api';
    const ORDER_API_BASE_URL   = 'http://4.147.69.124:8001/api';
    ...
    productListDiv.addEventListener('click', async (event) => {
        ...
        if (event.target.classList.contains('upload-btn')) {
            ...
            try {
                ...
                showMessage(`Image uploaded for ${updatedProduct.name}!`, 'success');
                fileInput.value = '';
                fetchProducts();
            } catch (error) {
